...
//======================================================================================================================
    ...
    tippy('.app-extended-menu', $.extend(tippySettingsTooltipButtonClick, {
        onShown: function(instance) {
            ...
            // Create archive
            $('.app-modal-create-archive-trigger').on('click', function() {
                initCreateArchive(this);
            });
            // Extract
            $('.app-modal-extract-trigger').on('click', function() {
                initExtract(this);
...
//======================================================================================================================
// Determine upload type by active tab
//======================================================================================================================

    $('#app-modal-upload .tabs a').on('click', function() {
        var $this = $(this);
        var $uploadType = $this.closest('form').find('input[name="upload_type"]');

        if ($this.attr('href') === '#tab-upload-local')
        {
            $uploadType.val('local');
        }
        else if ($this.attr('href') === '#tab-upload-url')
        {
            $uploadType.val('url');
        }
    });
...
//======================================================================================================================
    ...
    function initCreateArchive(trigger)
    {
        var config = readConfiguration(trigger);
        var $modal = $('#app-modal-create-archive');

        // Set values.
        $modal.find('.app-current-path').text(config.current_path);
        $modal.find('input[name="archive_name"]').val(config.id);
        $modal.closest('form').find('input[name="id"]').val(config.id);
